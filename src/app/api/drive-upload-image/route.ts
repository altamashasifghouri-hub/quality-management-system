...
export async function POST(req: Request) {
  ...
  try {
    ...
    const name = file.name || `evidence-${Date.now()}.jpg`;
    const mime = file.type || "image/jpeg";
    const folderId = driveFolderId("evidence");
    if (!folderId) return NextResponse.json({ error: "Evidence folder is not configured." }, { status: 500 });

    const boundary = `qms-ev-${Date.now().toString(36)}`;
